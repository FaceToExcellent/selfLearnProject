...
//MARK:2.内存管理基本原则
    ...
    /*autorelease 自动释放池 并不会直接释放，会在池子释放时将其内部的算有对象一一释放*/
    /*__strong 对象不会释放，引用计数加一*/
...
//MARK:7.类别的作用
    ...
    /*1.g给系统原有类添加方法
     2.分散类实现
     3.声明私有方法
     4.不能添加变量，property描述setter方法就不会报错*/
...
//MARK:9.KVO
    ...
    /*1.在既定的实体中，同一属性的所有值，具有相同的数据类型
     ...
     6.缺点：一旦使用KVC你的编译器无法检查处错误，不会对设置的键，键路径进行错误检查，并且执行效率低于合成存取器方法和自定的setter 和getter方法。因为使用KVC键值编码，它必须先解析字符串，然后在设置或者访问对象的实例变量
     */
...
//MARK: 23 iOS 开发中数据持久性有哪几种?
     ...
     *
*/
//MARK:24  中小大小的程序不会使用一下两个偏大的方式存储
    /**
...
//MARK:30堆和栈的区别
    /*1.栈区，由编译器自动分配释放，存放方法的参数值，局部变量的值等，栈是向低地址扩展的数据结构，是一块连续的内存区域。栈顶的地址和栈的最大容量是系统预先预定好的
     ...
     3.碎片问题：对于堆来说，频繁的new、delete必然会造成内存空间的不连续，从而造成大量的碎片，让程序效率降低，对于栈来说，则不会存在这个问题，因为栈是先进后出的队列，他们是以一对应的，所以不会有碎片
